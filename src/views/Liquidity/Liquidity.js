...
import useProvideMagikFtmLP from '../../hooks/useProvideMagikFtmLP';
...
const ProvideLiquidity = () => {
  ...
  const { onProvideTombFtmLP } = useProvideMagikFtmLP();
  ...
  const handleTombChange = async (e) => {
    ...
    setTombAmount(e.currentTarget.value);
    const quoteFromSpooky = await tombFinance.quoteFromSpooky(e.currentTarget.value, 'MAGIK');
  ...
  const handleTombSelectMax = async () => {
    const quoteFromSpooky = await tombFinance.quoteFromSpooky(getDisplayBalance(tombBalance), 'MAGIK');
      ...
      <Grid container justify="center">
        <Box style={{ width: '600px' }}>
          <Alert variant="filled" severity="warning" style={{ marginBottom: '10px' }}>
            <b>This and <a href="https://app.spiritswap.finance/#/"  rel="noopener noreferrer" target="_blank">SpiritSwap</a> are the only ways to provide Liquidity on MAGIK-FTM pair without paying tax.</b>
          </Alert>
                ...
                <Grid item xs={12} sm={12} style={{ borderRadius: 15 }}>
                  <Box p={4}>
                    <Grid container>
                      ...
                      <Grid item xs={12}>
                        <p>1 MAGIK = {tombPriceInFTM} FTM</p>
                        <p>1 FTM = {ftmPriceInTOMB} MAGIK</p>
